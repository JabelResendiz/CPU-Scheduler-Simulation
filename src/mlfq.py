...
#         self.running_process = None
#         level = self.process_event[process.pid]
...
#                 else:
#                     event_type = 'TIME_SLICE_EXPIRATION'
...
class MLFQScheduler(Scheduling):
    def __init__(self, quantums):
        super().__init__()
        self.quantums = quantums  # e.g. [4, 8]
        self.queues = [deque() for _ in quantums] + [deque()]  # última cola sin quantum = FCFS
        self.running_process = None
        self.process_levels = {}  # PID -> current level

    def add_process(self, process: Process):
        self.processes.append(process)
        self.process_levels[process.pid] = 0  # comienza en el nivel más alto
        arrival_event = Event(process.arrival_time, 'ARRIVAL', process)
        heapq.heappush(self.event_queue, arrival_event)
    ...
    def handle_time_slice_expired(self, process: Process):
        ...
        if level + 1 < len(self.queues):
            self.process_levels[process.pid] += 1  # degrada el proceso
        self.queues[self.process_levels[process.pid]].append(process)
        self.try_schedule_next()
