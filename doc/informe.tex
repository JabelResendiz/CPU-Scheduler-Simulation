...
\begin{itemize}
	...
	\item \textbf{Cola de eventos:} Se utilizó una \textit{priority queue} (cola de prioridad) para almacenar eventos, ordenados por tiempo.
	
	\begin{lstlisting}[language=Python, caption={Cola de eventos con heapq}]
		from heapq import heappush, heappop
		
		event_queue = []
		heappush(event_queue, (event.time, event))
		next_event = heappop(event_queue)[1]
	\end{lstlisting}
	
	\item \textbf{Simulación basada en eventos:} Se procesan los eventos en orden temporal. Al procesar un evento, se actualiza el estado del sistema y se generan nuevos eventos.
	
	\begin{lstlisting}[language=Python, caption={Ciclo principal de simulación}]
		while event_queue:
			current_event = heappop(event_queue)[1]
	...
	\item \textbf{Preempción:} En algoritmos como SRTF o MLFQ, se implementa lógica para interrumpir el proceso actual si uno de mayor prioridad llega.
	...
	\begin{lstlisting}[language=Python, caption={Tiempo de espera promedio}]
		average_waiting_time = sum(p.waiting_time for p in finished) / len(finished)
	\end{lstlisting}
